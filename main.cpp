...
void updateRaindrops()
{
    ...
    for (int i = 0; i < numRaindrops; i++)
    {
        ...
        if (raindropY[i] <= -200)
        {
            raindropX[i] = -600 + rand() % 1200;  // FIXED
            raindropY[i] = 284 + rand() % 100;    // top
        }
    }
}
...
void drawfishingRope(float x1, float y1, float x2, float y2)
{
    ...
    float m = fabs(dx) > fabs(dy) ? fabs(dx) : fabs(dy);
...
void updateHuman2()
{
    human2PosY += human2Speed;   // UP direction
...
<<<<<<< HEAD
    ...
    drawRLamp();
    drawLamp();
    drawLampLight();   // approx lamp light position
    drawBowl();
    drawBowl1();
